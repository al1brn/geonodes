...
class Domain:
    def __init__(self, data_socket, domain):
        ...
        self.bsocket     = data_socket.bsocket
        self.node        = data_socket.node
        self.domain      = domain
        self.fields      = {}
    ...
    def create_field_node(self, bl_idname, **kwargs):
        ...
        if node is None:
            node = nodes.create_node(bl_idname, **kwargs)
...
class Geometry(DataSocket):
    ...
    def reset_properties(self):
        ...
        type_name = type(self).__name__.split('.')[-1]
        ...
        if type_name == 'Mesh':
            ...
            self.cornes = Corners(self)
            
        elif type_name == 'Spline':
            self.splines = Curves(self)
