...
class Node:
    ...
    def fed_nodes(self):
        """ Get the set of nodes directly fed by this node
        ...
        for link in self.tree.links:
            if link.node0 == self:
                nodes.add(link.node1)
        return nodes
    
    def feeding_nodes(self):
        """ Get the set of nodes directly feeding this node
    ...
    def forwards(self, stop_on_zone=False):
        """ Iterate forwards
        ...
        for node in self.fed_nodes():
            nodes.add(node)

            if stop_on_zone and (node.bnode.bl_idname in ZONE_OUTPUTS):
                continue

            nodes = nodes.union(node.forwards(stop_on_zone))
        ...
        for link in self.tree.links:
            if link.node0 == self:
                ...
                if stop_on_zone and (link.node1.bnode.bl_idname in ZONE_OUTPUTS):
                    continue

                for node in link.node1.forwards(stop_on_zone):
                    yield node
        """

    def backwards(self, stop_on_zone=False):
        """ Iterate backwards
        ...
        for node in self.feeding_nodes():
            nodes.add(node)

            if stop_on_zone and (node.bnode.bl_idname in ZONE_INPUTS):
                continue

            nodes = nodes.union(node.backwards(stop_on_zone))
        ...
        for link in self.tree.links:
            if link.node1 == self:
                ...
                if stop_on_zone and (link.node0.bnode.bl_idname in ZONE_INPUTS):
                    continue

                for node in link.node0.backwards(stop_on_zone):
                    yield node
        """
    ...
    def get_linked_input_nodes(self):
        inodes = set()
        for node in self.backwards():
            if node.bnode.bl_idname in INPUT_NODES:
                inodes.add(node)
        return inodes
    
    def get_linked_output_nodes(self):
        onodes = set()
        for node in self.forwards():
            if node.bnode.bl_idname in OUTPUT_NODES:
                onodes.add(node)
        return onodes

    # ================================================================================
    # Get the relation with a zone
    # ================================================================================

    def get_zone_relation(self, input_node, output_node):
        """ Get the relation with a zone.

        A Node can be fed by a zone input node and feed a zone output node.
        The four possibilities are possible.

        If a Node is fed by zone input, it should also feed the output node. If it is not the case,
        the node is an error if it feeds another ouput node, otherwise it is a deadend.

        Fed by Input        Feed Output     Topology
        -------------------------------------------------------------------
        False               False           Node is independant of the zone
        False               True            Zone feeder
        True                False           Belongs to zone (with deadend or error)
        True                True            Belongs to zone
        """

        # IN PROGRESS

        # ---------------------------------------------------------------------------
        # Is the node fed by not zone input node ?
        # ---------------------------------------------------------------------------
        ...
        inodes = self.get_linked_input_nodes()
        onodes = self.get_linked_output_nodes()

        fed_by = input_node in inodes
        feeds  = output_node in onodes
...
class Frame(Node):
    ...
    # ====================================================================================================
    # Get the nodes directly fed by the frame
    # ====================================================================================================

    def fed_nodes(self):
        """ Get the set of nodes directly fed by this Frame
        ...
        nodes = set()
        for link in self.tree.links:
            if link.node0.is_child_of(self) and not link.node1.is_child_of(self):
                nodes.add(link.node1)
        return nodes
    
    # ====================================================================================================
    # Get the nodes directly feeding the frame
    # ====================================================================================================

    def feeding_nodes(self):
        """ Get the set of nodes directly feeding this Frame
        ...
        nodes = set()
        for link in self.tree.links:
            if link.node1.is_child_of(self) and not link.node0.is_child_of(self):
                nodes.add(link.node0)
        return nodes
    ...
    def forwards_OLD(self, stop_on_zone=False):
        """ Iterate forwards
    ...
    def backwards_OLD(self, stop_on_zone=False):
        """ Iterate backwards
...
class Zone():
    def __init__(self, tree, input_node, output_node):
        self.tree = tree

        self.input_node      = input_node
        self.output_node     = output_node

        # Internal nodes and feeders
        self.int_nodes   = {}
        self.feeders     = {}

        for node in input_node.forwards(stop_on_zone=True):
            self.int_nodes[node.bnode.name] = node

        for node in output_node.backwards(stop_on_zone=True):
            if node.bnode.name not in self.int_nodes:
                self.feeders[node.bnode.name] = node

    def __str__(self):
        return f"<Zone: {len(self.int_nodes)} nodes, {len(self.feeders)} feeders>"
...
class Tree(Frame):
    ...
    def new_zones_in_frames(self):
        """ Temporarily put zones into a frame
        """

        zones = []

        # ---------------------------------------------------------------------------
        # Loop on the nodes to get the Zone Input Nodes
        # ---------------------------------------------------------------------------
        ...
        for node in self.nodes.values():
            ...
            zones.append((zone_input, zone_output))
