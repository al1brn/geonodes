...
class Doc:
    def __init__(self, target='TEXT', width=100, margin=4, link_root=None):
        ...
        if target not in ['TEXT', 'MD']:
            raise NodeError (f"Unknown doc target '{target}'")
    ...
    def pop_bullets(self):
        if not len(self._bullets):
            raise NodeError("Doc.pop_bullets error: pop called on empty list. problem in balance with push_bullets and pop_bullets.")
        self._bullets.pop()
