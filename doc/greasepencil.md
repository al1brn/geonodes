...
``` python
GreasePencil(value=None, name=None, tip=None, panel='', hide_value=False, hide_in_modifier=False)
```
...
``` python
set_color(color=None, opacity=None, mode='STROKE')
```
...
``` python
set_color_fill(color=None, opacity=None)
```
...
``` python
set_color_stroke(color=None, opacity=None)
```
