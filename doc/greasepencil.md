...
``` python
GreasePencil(value: geonodes.core.socket_class.Socket = None, name: str = None, tip: str = '', panel: str = '', **props)
```
...
- **S** : [set_color](greasepencil.md#set_color) :black_small_square: [set_color_fill](greasepencil.md#set_color_fill) :black_small_square: [set_color_stroke](greasepencil.md#set_color_stroke) :black_small_square: [set_depth](greasepencil.md#set_depth) :black_small_square: [set_softness](greasepencil.md#set_softness) :black_small_square: [softness](greasepencil.md#softness) :black_small_square: [stroke_color](greasepencil.md#stroke_color)
...
``` python
merge_layers(mode: "Literal['MERGE_BY_NAME', 'MERGE_BY_ID']" = 'MERGE_BY_NAME')
```
...
#### Arguments:
- **mode** (_Literal['MERGE_BY_NAME', 'MERGE_BY_ID']_ = MERGE_BY_NAME) : parameter 'mode' in ('By Name', 'By Group ID')
...
``` python
merge_layers_by_id(group_id: 'Integer' = None)
```
...
``` python
named_layer_selection(name: 'String' = None)
```
...
``` python
set_color(color: 'Color' = None, opacity: 'Float' = None, mode: "Literal['STROKE', 'FILL']" = 'STROKE')
```
...
- **mode** (_Literal['STROKE', 'FILL']_ = STROKE) : parameter 'mode' in ('Stroke', 'Fill')
...
``` python
set_color_fill(color: 'Color' = None, opacity: 'Float' = None)
```
...
``` python
set_color_stroke(color: 'Color' = None, opacity: 'Float' = None)
```
...
``` python
set_depth(depth_order: "Literal['2D', '3D']" = '2D')
```
...
#### Arguments:
- **depth_order** (_Literal['2D', '3D']_ = 2D) : parameter 'depth_order' in ('2D Layers', '3D Location')
...
``` python
set_softness(softness: 'Float' = None)
```
...
``` python
to_curves(layers_as_instances: 'Boolean' = None)
```
