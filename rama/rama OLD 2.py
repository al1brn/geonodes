...
island_x_length     =  4.   # Length is from North to South
island_y_length     = 10.   # Length along the circle
island_altitude     =   .3  # Altitude of the island
island_internal     =   .95 # To compute the island cliff slope


# ----- Computed parameters

cyl_circ            = cyl_radius*2*np.pi
rama_ymax           = cyl_circ / 2

cyl_x0              = -cyl_length/2
cyl_x1              =  cyl_length/2

north_radius        = cyl_radius - north_altitude
south_radius        = cyl_radius - south_altitude
...
# Island is drawn with two arcs of circle.
# An arc is such as the max distance between the circumference and the segment is d = 2 km (island_x_length/2)
# Let a be half the angle of the arc, and s half the length of segment:
# - s = r.sin(a)
# - d/s = tan(a)
#
# We want a and d such as:
# - d = 2 km (island_x_length/2)
# - s = 5 km (island_y_length/2)

island_alpha        = np.arctan(island_x_length/island_y_length)
island_radius       = island_y_length/2/np.sin(island_alpha)/2
...
# ====================================================================================================
# Get a rama cylinder section

def get_cylinder_section(factor=1, scale=1000, select=None):
    
    if isinstance(select, str):
        select = {select}
        
    # ----------------------------------------------------------------------------------------------------
    # A cylinder section from profile
    
    def hemi(x, y, x0, alt0, x_inter, alt_inter, x1, alt1):
        
        if not hasattr(x, '__len__'):
            return hemi(np.array([x]), np.array([y]), x0, alt0, x_inter, alt_inter, x1, alt1)[0]
        
        z = np.empty_like(x)
        
        sel = x <= x_inter
        z[sel] = alt0 + (x[sel] - x0) * ((alt_inter - alt0)/(x_inter - x0))
        
        sel = np.logical_not(sel)
        z[sel] = alt_inter + (x[sel] - x_inter) * ((alt1 - alt_inter)/(x1 - x_inter))
        
        vs = np.stack((x, y, z)).T

        return to_cylinder(vs, factor=factor, scale=scale)
    ...
    def island(x, y):
        ...
        if not hasattr(x, '__len__'):
            return island(np.array([x]), np.array([y]))
        
        z = np.ones_like(x)*-.2
        #v = np.stack((x, y)).T
        
        r0 = island_radius
        r1 = r0*island_internal
        
        for i in range(2):
            
            if i == 0:
                sel = x>=0
                cx  = -r0 + island_x_length/2
            else:
                sel = x<0
                cx  = r0 - island_x_length/2
                
            v  = np.stack((x[sel], y[sel])).T
            z_ = np.ones_like(x[sel]) * bottom

            ds = np.linalg.norm(v - [cx, 0], axis=-1)
            sel0 = ds <= r0
            sel1 = ds <= r1
            
            z_[sel1] = island_altitude
            sel_slope = np.logical_and(np.logical_not(sel1), sel0)
            z_[sel_slope] = island_altitude + (ds[sel_slope] - r1)*((bottom - island_altitude)/(r0 - r1))
    ...
    if select is None or name in select:
        ...
        nx = int(north_length)*3  * resolution
        ny = int(north_radius)*20 * resolution
        ...
        func = lambda x, y: hemi(x, y, north_x0, north_altitude, north_x1, north_altitude, sea_x0, -.01)
        cylinder[name] = MultiResGrid(nx, ny, bounds=((north_x0, -rama_ymax), (sea_x0, rama_ymax)), func=func, cuts=cuts)
    ...
    if select is None or name in select:
        ...
        nx = int(south_length)*3  * resolution
        ny = int(south_radius)*20 * resolution
        ...
        func = lambda x, y: hemi(x, y, sea_x1, -.01, south_x0, south_altitude, south_x1, south_altitude)
        cylinder[name] = MultiResGrid(nx, ny, bounds=((sea_x1, -rama_ymax), (south_x1, rama_ymax)), func=func, cuts=cuts)
    ...
    if select is None or name in select:
        ...
        cuts = [
            {'axis': 'x', 'at': sea_x0, 'to': sea_x0 + .3, 'cuts': 15},
            {'axis': 'x', 'at': sea_x1 - .3, 'to': sea_x1, 'cuts': 15},
            ]
        
        func = lambda x, y: hemi(x, y, sea_x0, 0, (sea_x0+sea_x1)/2, 0, sea_x1, 0)
        cylinder[name] = MultiResGrid(nx, ny, bounds=((sea_x0, -rama_ymax), (sea_x1, rama_ymax)), func=func, cuts=cuts)
    ...
    if select is None or name in select:
        ...
        nx = int(island_x_length)*30
        ny = int(island_y_length)*30
        ...
        cylinder[name] = MultiResGrid(nx, ny, bounds=((-island_x_length/2, -island_y_length/2), (island_x_length/2, island_y_length/2)), func=island, cuts=cuts)
