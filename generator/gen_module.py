...
from generator.gen_nodes import Socket, Sockets, Node, SOCKET_TYPES
...
def get_folder(folder_name):
    ...
    # ----- The path of the current files : ...../geonodes/generator/gen_module.py
    
    file_path = Path(__file__)

    # ----- Geonoders folder
    
    mod_folder = file_path.parent.parent
    
    # ----- To the requested folder
    
    folder = mod_folder.joinpath("geonodes").joinpath(folder_name)

    # ----- Checks
    
    if not (folder.exists() and folder.is_dir()):
        raise RuntimeError(f"Impossible to find the folder {folder} from file path {file_path}.")
...
def gen_module():
    ...
    with open(get_folder("nodes").joinpath("nodes.py"), 'w') as f:
        ...
        f.write("from geonodes.core.node import Socket, SocketIn, Sockets, Node, Attribute\n\n")
    ...
    for i, dgen in enumerate(gsock.DATA_CLASSES):
        ...
        with open(get_folder("sockets").joinpath(f"{class_gen.class_name.lower()}.py"), 'w') as f:
            ...
            f.write("from geonodes.core import socket as bcls\n")
    ...
    for blid, node in all_nodes.items():
        if not blid in gsock.GEN_NODES:
...
def gen_module_OLD():
    ...
    print(get_folder("sockets"))
    print(get_folder("nodes"))
    ...
    print(os.path.relpath("./classes", start=__file__))
    print()
