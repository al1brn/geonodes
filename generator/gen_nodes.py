...
class Node:
    ...
    def gen_socket_property(self):
        ...
        def line(index, is_get=True):
            if is_get:
                return f"return Node.{self.inputs[index].class_name}(self.inputs[{index}])\n" #".link\n"
            else:
    ...
    def build_arguments(self, only_enabled=False, variation={}, self_name=None, attribute=False, domain=None, data_type=None, hooks={}):
        ...
        # ---------------------------------------------------------------------------
        # Attribute complementory arguments
        
        if attribute:
            args.append(Argument('self_owner', 'SELF', class_name='Param', arg_type='VARIATION'))
            
            arg_type = 'PARAMETER' if domain is None else 'VARIATION'
            args.append(Argument('domain', domain, class_name='Param', arg_type=arg_type))
            
            args.append(Argument('data_type', data_type, class_name='Param', arg_type='VARIATION'))
    ...
    def gen_node_class(self, is_attribute=False):
        ...
        yield _2_ + "self.out_socket_names = ["
        sep = ""
        ...
        if self.outputs:
            ...
            for uname, spec in uniques.items():
                ...
                if type(spec) is int:
                    ...
                    yield _2_ + f"return self.{self.outputs[spec].class_name}(self.outputs[{spec}])\n\n"
                    ...
                    for index, conditions in spec.items():
                        ...
                        yield _3_  + f"return self.{self.outputs[index].class_name}(self.outputs[{index}])\n"
                        
                    yield _2_ + "self.check_parameters()\n\n"
                    
        yield _1_ + "@property\n"
        yield _1_ + "def output(self):\n"
        if len(self.outputs) == 0:
            yield _2_ + "return self\n\n"
        elif len(uniques) == 1:
            yield _2_ + f"return self.{self.out_name}\n\n"
        else:
            yield _2_ + "return self.output_sockets()\n\n"
